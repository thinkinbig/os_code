...
/**
 * Creates a new array and copies length elements from src array into the new
 * array. The caller needs to free the new returned array.
 */
...
/**
 * Merge src array which contains two sorted sub array src[start...middle - 1]
 * and src[middle...end-1]. The sub arrays are merged into destToMergeInto with
 * the result sorted as well.
 */
void merge(int *src, size_t start, size_t middle, size_t end,
           int *destToMergeInto) {
  ...
  for (size_t i = start; i < end; ++i) {
    if (leftIndex < middle &&
        (rightIndex >= end || src[leftIndex] <= src[rightIndex])) {
