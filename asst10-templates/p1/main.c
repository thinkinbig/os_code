...
static void pushAround(MutexAndInt *from, MutexAndInt *to)
{
    pthread_mutex_t *mutexv[2] = {&from->mutex, &to->mutex};

    for (int i = 0; i < PUSH_TIMES; i++) {
        if (multi_mutex_lock(mutexv, 2) != 0) {
            printf("multi_mutex_lock failed\n");
            return;
        }
        int f = from->value;
        int t = to->value;
...
int main()
{
    ...
    for (int i = 0; i < VALUES; i++) {
        pushAround(&values[i], &values[(i+1) % VALUES]);
    }

    test_equals_int(printMutexAndInt(values, VALUES), INITIAL_VALUE * VALUES, "sum of values is correct after multi_mutex_lock");
    ...
    test_equals_int(printMutexAndInt(values, VALUES), INITIAL_VALUE * VALUES, "sum of values is correct after multi_mutex_trylock");
